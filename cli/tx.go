...
import (
	...
	"github.com/davidmovas/chain-forge/chain"
...
func grpcTxSign(
	...
	req := &rpc2.TxSignReq{From: from, To: to, Value: value, Password: ownerPass}
...
func grpcTxSearch(
	...
	cln := rpc2.NewTxClient(conn)
	req := &rpc2.TxSearchReq{Hash: hash, From: from, To: to, Account: account}
...
func txSearchCmd(ctx context.Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "search",
		Short: "Searches transactions by the transaction hash, from, to, and account address",
		RunE: func(cmd *cobra.Command, _ []string) error {
			addr, _ := cmd.Flags().GetString("node")
...
func txProveCmd(ctx context.Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "prove",
		Short: "Receives Merkle proof and Merkle root for transactions hash",
		RunE: func(cmd *cobra.Command, _ []string) error {
			addr, _ := cmd.Flags().GetString("node")
			hash, _ := cmd.Flags().GetString("hash")
...
func txVerifyCmd(ctx context.Context) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "verify",
		Short: "Verifies Merkle proof for transactions hash against Merkle root",
		RunE: func(cmd *cobra.Command, _ []string) error {
			addr, _ := cmd.Flags().GetString("node")
			hash, _ := cmd.Flags().GetString("hash")
