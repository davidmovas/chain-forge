...
func createStateSync(
	ctx context.Context, peerReader node2.PeerReader, bootstrap bool,
) (*chain2.State, error) {
	var nodeCfg node2.NodeCfg
	if bootstrap {
		nodeCfg = node2.NodeCfg{
	...
	stateSync := node2.NewStateSync(ctx, nodeCfg, peerReader)
	return stateSync.SyncState()
}

func createBlocks(
	keyStoreDir, blockStoreDir string, gen chain2.SigGenesis, state *chain2.State,
) error {
