...
func createPeerDiscovery(
	...
	var peerDiscCfg node2.PeerDiscoveryCfg
	if bootstrap {
		peerDiscCfg = node2.PeerDiscoveryCfg{NodeAddr: bootAddr, Bootstrap: true}
	} else {
		peerDiscCfg = node2.PeerDiscoveryCfg{NodeAddr: nodeAddr, SeedAddr: bootAddr}
	}
	peerDisc := node2.NewPeerDiscovery(ctx, wg, peerDiscCfg)
	if start {
		wg.Add(1)
...
func createEventStream(
	...
	evStream := node2.NewEventStream(ctx, wg, 10)
	wg.Add(1)
...
func TestPeerDiscover(t *testing.T) {
	...
	req := &rpc2.PeerDiscoverReq{Peer: nodeAddr}
...
func TestStreamSubscribe(t *testing.T) {
	...
	conn := grpcClientConn(t, func(grpcSrv *grpc.Server) {
		node := rpc2.NewNodeSrv(nil, evStream)
		rpc2.RegisterNodeServer(grpcSrv, node)
	})
	// Create the gRPC node client
	cln := rpc2.NewNodeClient(conn)
